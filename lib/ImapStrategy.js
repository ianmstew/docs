...
// Have to do this because PassportService isn't defined yet when app.get is called a few lines from here.
function ensureAuthenticated( req, res, next ) {
	return PassportService.ensureAuthenticated( req, res, next );
}
	...
	app.get( 
		...
		function( req, res ) {
			var options = { 
				...
				imapConfig: self.config,
				layout: 'wwwlayout/jquery.ejs' };
			
			if( self.config.host )
				options.host = self.config.host;
			if( self.config.port )
				options.port = self.config.port;
			if( self.config.secure )
				options.secure = self.config.secure;
			if( self.config.immutableSettings )
				options.immutableSettings = self.config.immutableSettings;

			if( req.query.failed ) {
				if( req.query.message )
					options.message = req.query.message;
				else
					options.message = 'Connection to IMAP server failed.';
				...
				if( req.query.secure )
					options.secure = req.query.secure;
			}
			res.render( 'imapLogin', options );
	});
	...
	this._verify = function( req, username, password, host, port, secured, done ) {
		...
		else 
		{
			dns.lookup( host, function( error, address, family ) 
			{
				...
				{
					connectionData.owner = req.user.identifier;
					ImapService.testAuthentication( 
						req.user.identifier, connectionData, 
